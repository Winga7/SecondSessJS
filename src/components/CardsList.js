...
export const CardsList = (element, items, itemTemplate, searchableFields) => {
	...
	const filterAndPaginate = (perPage = 12) => {
		...
		if (value !== "") {
			filteredItems = items.filter((item) => searchableFields.some((field) => item[field] && item[field].toString().toLowerCase().includes(value)));
		} else {
			filteredItems = items;
		}
	...
	window.addEventListener("popstate", () => {
		currentPage = parseInt(new URL(window.location).searchParams.get("page")) || 1;
		filterAndPaginate();
	});
};
